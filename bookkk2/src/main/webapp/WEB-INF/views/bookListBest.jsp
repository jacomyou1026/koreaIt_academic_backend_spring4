		  ...
		  <form role="form" method="get">    
						...
						<c:if test="${list.size() > 0}">
					               <div class="bookList_desc">
                                        <span>베스트 셀러</span>
                                        <p>베스트셀러 코너는 최근 가장 많이 팔린 인기 넘치는 책들을 선별해놓은 코너입니다</p>
                                    </div>
                   <c:forEach items="${list}" var="list">
                                    <div class="booklist">
                                            ...
                                            <div class="list_detail">
                                                <a href="" id="title">${list.subject}</a> <span id="catenamed">	&#91;${list.cateName}&#93;</span>
                       ...
                       <c:if test="${list.size() > 0}">      
						...
						<ul class="pagination">
							<c:if test="${pc.prev}">
								<li><a href="bookListNew?page=${pc.startPage - 1}&countPerPage=${pc.paging.countPerPage}">이전</a></li>
							</c:if> 
							
							<c:forEach begin="${pc.startPage}" end="${pc.endPage}" var="idx">
								<li <c:out value="${pc.paging.page == idx ? 'class=info' : ''}" />>
								<a href="bookListNew?page=${idx}&countPerPage=${pc.paging.countPerPage}">${idx}</a></li>
							</c:forEach>
							
							<c:if test="${pc.next && pc.endPage > 0}">
								<li><a href="bookListNew?page=${pc.endPage + 1}&countPerPage=${pc.paging.countPerPage}">다음</a></li>
							</c:if> 
						</ul>
					</div>
					</c:if>
