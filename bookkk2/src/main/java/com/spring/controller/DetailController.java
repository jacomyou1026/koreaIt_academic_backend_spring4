...
public class DetailController {
	...
	public String bookDetail(Model model, String page, Integer num) {
		...
		//전체 게시물 수 구하기
		int row_total = review_dao.rowTotal();
		
		//하단부에 추가될 페이지 메뉴 생성
		String pageMenu = Paging.getPaging("bookDetail.do",//호출되는 페이지 url 경로
											nowPage, //현재 페이지 번호
											row_total,//전체 게시물 수
	...
	public String review_write(int bookNum, String id, String content, Integer rating) { //bookNum 받아와야함.. 
		...
		review_vo.setIp(ip);
		review_vo.setPurchaseNum(bookNum);
		review_vo.setId(id);
	...
	//대댓글 작성
		...
		@ResponseBody
		public String comment_write(Integer purchaseNum, String id, String content, Integer rating, Integer reviewNum) { //bookNum 받아와야함.. 
			...
			review_vo.setPurchaseNum(purchaseNum);
			review_vo.setId(id);
			review_vo.setContent(content);
