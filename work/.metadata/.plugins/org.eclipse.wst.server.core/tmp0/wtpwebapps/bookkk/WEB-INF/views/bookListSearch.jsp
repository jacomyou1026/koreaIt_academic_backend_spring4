		  ...
		  <form role="form" method="get">    
                   ...
                   <c:forEach items="${list}" var="list">
                       ...
                       <c:if test="${list.size() > 0}">      
						...
						<ul class="pagination">
							<c:if test="${pc.prev}">
								<li><a href="bookListSearch?page=${pc.startPage - 1}&countPerPage=${pc.paging.countPerPage}">이전</a></li>
							</c:if> 
							
							<c:forEach begin="${pc.startPage}" end="${pc.endPage}" var="idx">
								<li <c:out value="${pc.paging.page == idx ? 'class=info' : ''}" />>
								<a href="bookListSearch?page=${idx}&countPerPage=${pc.paging.countPerPage}">${idx}</a></li>
							</c:forEach>
							
							<c:if test="${pc.next && pc.endPage > 0}">
								<li><a href="bookListSearch?page=${pc.endPage + 1}&countPerPage=${pc.paging.countPerPage}">다음</a></li>
							</c:if> 
						</ul>
					</div>
					</c:if>
