...
<link rel="stylesheet" href="resources/css/bookList/jungo/jungoSearch.css">
</head>
<body>
    ...
    <%@include file="../../include/jungoheader.jsp" %>	
		...
		<div class="content_area">
    		...
    		<%@include file="../../include/jungo_nav.jsp" %>   
    		 ...
    		 <%@include file="../../include/asideJungoSearch.jsp" %>	
						  <div class ="content">
                            <div id ="main_box">
                   ...
                   <c:forEach items="${list}" var="list">
                                    <div class="booklist">
                                            ...
                                            <div class="list_detail">
       											<a href="/jungosearchview?JbookNum=${list.jbookNum}&page=${pc.paging.page}&countPerPage=${pc.paging.countPerPage}&keyword=${param.keyword}&condition=${param.condition}" id="title">${list.jsubject}</a> <span id="catenamed">	&#91;${list.cateName}&#93;</span><span id="jungo_desc">${list.id} 님의 중고도서</span>                                                   
					...
					<div 	class="col-md-offset-3">
						<ul class="pagination">
							<c:if test="${pc.prev}">
								<li><a href="jungoSearch?page=${pc.startPage - 1}&countPerPage=${pc.paging.countPerPage}&keyword=${param.keyword}&condition=${param.condition}">이전</a></li>
							</c:if> 
							
							<c:forEach begin="${pc.startPage}" end="${pc.endPage}" var="idx">
								<li <c:out value="${pc.paging.page == idx ? 'class=active' : ''}" />>
								<a href="jungoSearch?page=${idx}&countPerPage=${pc.paging.countPerPage}&keyword=${param.keyword}&condition=${param.condition}">${idx}</a></li>
							</c:forEach>
							
							<c:if test="${pc.next && pc.endPage > 0}">
								<li><a href="jungoSearch?page=${pc.endPage + 1}&countPerPage=${pc.paging.countPerPage}&keyword=${param.keyword}&condition=${param.condition}">다음</a></li>
							</c:if> 
						</ul>
					</div>
					</c:if>
